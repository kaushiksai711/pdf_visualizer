...
// import { fetchAndProcessEmbeddings, reduceToThreeDimensions, clusterEmbeddings } from '../services/visualizationService';
...
//             } else if (sphere.userData.isSelected) {
//               // Keep selected spheres highlighted
//               sphere.material.emissive.setHex(0x888888);
//             } else {
//               sphere.material.emissive.setHex(0x000000);
//             }
...
//             <p>{selectedChunkDetails.keywords ? selectedChunkDetails.keywords.join(', ') : 'No keywords available'}</p>
//           </div>
//           <button 
        ...
        const onClick = async (event) => {
          ...
          if (intersects.length > 0) {
            ...
            try {
                ...
                console.log(`${selectedDocs},${clicked.userData.chunkIndex}|| 0`)
              const response = await fetch(
                `http://localhost:5000/api/chunks?file_name=${selectedDocs[0].name}&chunk_index=${clicked.userData.chunkIndex}`
              );
